...
## 11. パイプで受けると、ダッシュボードを閉じた瞬間に子が死ぬ（2026-09-07）

実測。親が `spawn(..., { stdio: ['ignore','pipe','pipe'] })` で受けている子は、
親が終了した時点で読み手が居なくなり、子は次に stdout へ書いたところで死ぬ。
`detached` を付けていても、パイプである限り同じ。

```
親を落とす → child.log は親の生存中の 3 行で止まる → pgrep に残らない
```

つまり「ダッシュボードを閉じてもエージェントは裏で動き続けている」は成り立たない。
続けさせるには、出力をパイプではなくファイルに書かせ、親はそれを追いかける。

- `stdio: ['ignore', <fd>, <fd>]` + `detached: true` + `child.unref()`
- 親はファイルの続きを読む（Tail）。終了はプロセスの生死で判定する
- 立ち上げ直したら、控えておいた PID が生きているかを見て、同じファイルを頭から読み直す

実測で、10 行出す子を 3 行読んだところで親を落としても子は走り切り、
追いかけ直して 12 行すべて拾えることを確認した。


## 12. codex の使用量は「最後に動かしたときの記録」（2026-09-08）

`rate_limits` はセッション記録（rollout）に書き込まれた時点の値でしかない。
codex を動かさなければ更新されないので、窓が入れ替わったあとも古い数字が残る。

実例。9/7 23:58 に 5 時間窓が 99% に達し、その窓は 9/8 04:16 にリセットされた。
ところが次に codex が動いたのは 9/8 14:54 で、その間の約 15 時間、
いちばん新しい記録は「99%・リセット時刻 04:16」のままだった。
そのまま出すと、上限に張り付いて動かないように見える。

対応:

- `resets_at` を過ぎた窓は前の窓のものとして 0% 扱いにし、`expired` を立てて
  画面には「(リセット済)」と出す
- 記録を選ぶときはファイルの更新時刻ではなく `resets_at` の新しさで選ぶ。
  長く続いているスレッドのファイルが更新されても、そこに載っている使用量が
  最新とは限らない（実際、7/25 に始まったスレッドのファイルが
  いちばん新しい更新時刻を持っていた）


## 13. トークン消費の内訳（2026-09-09 実測）

codex の消費が急増したと報告があり、`~/.codex/sessions` の全ロールアウトを集計した。

```
273 ターン / 入力計 27,805,636（うちキャッシュ 87%）
1 ターン平均 101,852   中央値 82,690   最大 238,190
空ディレクトリで 1 ターン投げたときの下地 16,469
```

5 時間の枠は**実作業 28〜71 分**で使い切られていた（3 つの窓で確認）。

### 効いている順

1. **会話の積み上がり**が支配的。下地は 16k しかなく、残りは全部履歴の送り直し。
   1 セッション内で 23,850 → 39,921 → 57,168 → 75,056 → 92,252 と 5 ターンで 4 倍になり、
   長いものは 238k（文脈上限に近い）に達していた。
2. **巨大な文脈でのキャッシュミス**。全体では 87% 乗っているが、23 万トークンの
   ターンで 3〜5% しか乗らない例が複数あった。codex は毎ターン `world_state`
   （24,809 文字）と `AGENTS.md` を読み直して先頭付近に入れるため、
   **作業中にルールファイルを編集すると以降が丸ごと外れる**。
3. `approvals_reviewer = "auto_review"` が別モデル（`codex-auto-review`）を呼ぶ。
   69 ターン・366 万トークン（13%）。→ ただし切ってはいけない、下記。
4. 機能フラグ（skills / plugins / multi_agent / apps）の寄与は小さい。
   全部切って測って 16,469 → 14,955。1.5k しか減らない。
5. 推論トークン自体はほとんど出ていない（astra/high の 17 ターンで 487）。
   `model_reasoning_effort` が効くのは量ではなく単価。

### ★auto_review は安全装置。切らないこと★

費用だけ見て「使っていない機能」と判断しかけたが、中身を読むと**承認が要る操作の
リスク判定**をしていた。7/4 以降 1,042 回動作し、5 件を拒否している。

- 機密ログ（IP・メール・UA）を未特定の S3 へ自動送信し、ローカルを削除する変更
- `tmux kill-window` で未保存の作業ごとプロセスを落とす操作
- 本番の未使用イメージ全削除（ロールバック用を失う）× 2
- 本番での自動削除・ログ転送を伴う永続的な運用変更

`exec` では対話的な承認が出ないぶん、ここが唯一の歯止めになっている。13% はその費用。

### 質を落とさずに効く手

- **スレッドを短く保つ**（`N` の引き継ぎ）。16k から始め直せるので効果がいちばん大きい
- **作業中にルールファイルを編集しない**。キャッシュが外れる
